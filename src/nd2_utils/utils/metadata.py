...
class MetadataHandler:
    ...
    def convert_attrs_to_dict(attrs_obj: Any) -> Dict[str, Any]:
        ...
        if hasattr(attrs_obj, "__dict__"):
            ...
            try:
                ...
                logger.debug(
                    f"Converted using dict(), got {len(attributes_dict)} items"
                )
            except (TypeError, ValueError):
                attributes_dict = {"raw_object": str(attrs_obj)}
                logger.warning(
                    "Could not convert attrs to dict, using string representation"
                )
    ...
    @staticmethod
    def build_info_dict(
        file_path: str, xarray, attrs: Dict[str, Any]
    ) -> Dict[str, Any]:
